...
public:
	...
	int minDepth(TreeNode *root) {
		return minDepthImpl(root, false);
	}

	int minDepthImpl(TreeNode *root, bool has_brother) {
		if (!root) return has_brother ? INT_MAX : 0;

		int left = minDepthImpl(root->left, root->right != nullptr);
		int right = minDepthImpl(root->right, root->left != nullptr);
