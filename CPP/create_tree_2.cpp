...
public:
	...
	TreeNode *buildTree2(vector<int> &inorder, vector<int> &postorder) {
		// for_each(inorder.begin(), inorder.end(),
		// [] (int x) { cout << " element " << x << endl; });

		//	for_each(postorder.begin(), postorder.end(),
		//	[] (int x) { cout << " element " << x << endl; });

		return createTree2(inorder.begin(), inorder.end(),
							postorder.begin(), postorder.end());
	}

	template<typename InputIterator>
	TreeNode* createTree2(InputIterator in_first, InputIterator in_last,
		...
		// this will work too
		// auto in_pos = find(in_first, in_last, root->val);
		auto left_size = distance(in_first, in_pos);

		root->left = createTree2(in_first, in_pos, post_first, next(post_first, left_size));
		root->right = createTree2(next(in_pos), in_last, next(post_first, left_size), prev(post_last));
...
//	int **ary = new int*[100];
	...
	vector<int> v1{1, 2};
	vector<int> v2{2, 1};
	TreeNode *tn = pp->buildTree2(v1, v2);
